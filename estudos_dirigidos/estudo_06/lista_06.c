...
void metodo01 ( void )
{
 ...
 // printf ( "\ntamanho = %d\n", tamanho );
 // repetir para cada simbolo (mapear)
    for ( x=0; x<tamanho; x=x+1 )
    {
     // obter um simbolo      (decompor/separar)
        simbolo = palavra [ x ];
     //	printf ( "\n%d %c", x, simbolo );
     // testar se maiuscula   (filtrar)
...
void metodo09 ( void )
{
 ...
 // mapear valores
    for ( x=1; x<=n; x=x+1 )
    {
     ...
     // a*x**2+b*x**1+c*x**0 = 1*x**2+0*x**1+1*x**0
        y = x*x + 1;
        printf ( "\n%d %d", x, y );
    } // end for
    printf ( "\n" );
